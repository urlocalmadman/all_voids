...
let emotions = { Abandonment: 5, Angry: 5, Anxious: 5, Cringe: 1, Curiosity: 5, DownBad: 5, Envy: 5, Estrangement: 5, Gluttony: 5, Greed: 5, Grumpy: 5, Jealous: 5, Judgement: 5, Lonely: 5, Lust: 5, Nostalgia: 5, Pain: 5, Paranoia: 5, Pride: 5, Sad: 5, Sadge: 4, Salty: 5, Sanctimony: 5, Sloth: 5, Sonder: 5, Spite: 5, Wrath: 5 };

Object.keys(emotions).forEach(emotion => {
    for (let i = 1; i < emotions[emotion] + 1; i++) {
        let pet = fs.readFileSync(`./all_normal_svgs/${emotion}/${i}.txt`, 'utf-8').toString();
        // console.log(pet);
        pet = pet.replace('width=300', 'width=1200').replace('height=300', 'height=1200');
        Object.keys(vivids).forEach(vivid => {
            console.log(`${vivid} ${emotion} ${i}`);
            sharp(Buffer.from(pet.replaceAll('black', vivids[vivid][0]).replaceAll('white', vivids[vivid][2]).replaceAll('#3E3E60', vivids[vivid][1]))).toFile(`./${vivid}/${emotion}/${i}.png`, function (err) {});
        });
    }
});
