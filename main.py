print('DEPRECATED ONLY FOR ARCHIVE PURPOSE')
...
# emotions = {'Abandonment': 5, 'Anger': 5, 'Anxious': 5, 'Cringe': 1, 'DownBad': 5, 'Envy': 5, 'Estrangement': 5, 'Gluttony': 5, 'Greed': 5, 'Grumpy': 5, 'Jealous': 5, 'Judgement': 5,
...
# for x in variants:
#     os.mkdir(fr'.\{variants[x]}')
#     for i in emotions:
#         os.mkdir(fr'.\{variants[x]}\{i}')
#         for j in range(1, emotions[i]+1):
#             img = requests.get(
#                 f'https://img.voidpet.com/thumbnail/{i}{j}-{x}.jpg')

#             with open(fr'.\{variants[x]}\{i}\{j}.jpg', 'wb') as f:
#                 f.write(img.content)
